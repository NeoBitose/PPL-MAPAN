...
@section('content')
      ...
      @foreach ($data as $item)
        ...
        <div class="card-postingan" id="{{ $item['id_postingan'] }}">
          ...
          <div class="gambar-postingan">
            <img src="@if($item['gambar_postingan'] != "") {{ asset('img/postingan/' . $item['gambar_postingan']) }} @endif" alt="">
          </div>
          <div class="komentar">
            <div class="">
              ...
              <div class="panel">
                ...
                @auth
                @if (auth()->user()->role == 'user')
                <form action="/komentar" method="post" class="mt-3 add-komentar add-komentar-{{$noind}}">
                  @csrf
                ...
                <form action="/komentar" method="post" class="mt-3 edit-komentar edit-komentar-{{$noind}}">
                  @csrf
                  @method('PUT')
                  <input type="hidden" name="id_postingan" id="id_postingan-input-{{$noind}}" value="">
                  <input type="text" name="komentar" id="komentar-input-{{$noind}}" value="">
                  <button class="btn-comment">Edit Komentar</button>
                </form>
                @endif
                @endauth
                              ...
                              <div class="box-komentar">
                                <div class="flex">
                                  ...
                                  <a href="#edit-komentar"
                                    onclick="swap_form({{$noind}},{{ $item['id_postingan'] }}, {{ $item['komentar'][$i]->id_komentar }}, '{{ $item['komentar'][$i]->komentar }}')"
                                    class="m-0">
  ...
  <script>
    ...
    add_komentar.style.display = "flex";
    edit_komentar.style.display = "none";

    function swap_form($formid, $id, $idkomentar, $komentar) {
      document.querySelector(".add-komentar-"+$formid).style.display = "none";
      document.querySelector(".edit-komentar-"+$formid).style.display = "flex";
      document.querySelector(".edit-komentar-"+$formid).action = "/komentar/" + $idkomentar;
      var id_post_input = document.getElementById("id_postingan-input-"+$formid);
      id_post_input.value = $id;
      var komen_input = document.getElementById("komentar-input-"+$formid);
      komen_input.value = $komentar;
    }
  </script>
@endsection
